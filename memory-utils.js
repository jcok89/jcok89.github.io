window.MemoryTrainer = (() => {
  ...
  const load = () => {
    try {
      return JSON.parse(localStorage.getItem(KEY)) || { wordStats: {}, sessions: [], reviews: [] };
    } catch (e) {
      return { wordStats: {}, sessions: [], reviews: [] };
    }
  };
  const save = data => localStorage.setItem(KEY, JSON.stringify(data));
  const norm = value => String(value || '').trim().replace(/\s+/g, '');
  const wordOf = item => norm(typeof item === 'string' ? item : item.word);
  const dateKey = value => new Date(value || Date.now()).toISOString().slice(0, 10);
  ...
  function reviewWords(program) {
    return [...new Set(dueReviews(program).map(r => r.word))];
  }
  ...
  function weightedPool(items, program) {
    ...
    items.forEach(item => {
      ...
      let weight = 1;
      weight += Math.min(6, stat.missed || 0);
      weight += Math.min(4, stat.weight || 0);
  ...
  function recordSession(program, items, correctWords, extra = {}) {
    ...
    uniqueWords.forEach(word => {
      ...
      if (correctSet.has(word)) {
        ...
        data.reviews.forEach(r => {
          if (r.word === word && r.program === program && !r.done) r.done = true;
        });
      } else {
        stat.missed += 1;
        stat.weight = Math.min(12, (stat.weight || 0) + 3);
        [1, 3, 7].forEach(days => {
          data.reviews.push({ word, program, dueAt: addDays(days), days, done: false, createdAt: now });
        });
      }
    ...
    data.sessions.push({
      ...
      score: uniqueWords.length ? Math.round((correct.length / uniqueWords.length) * 100) : 0,
      words: uniqueWords,
      missedWords: missed,
      extra
    });
  ...
  function history(program, limit = 20) {
    return load().sessions.filter(s => !program || s.program === program).slice(-limit);
  }
  ...
  function drawChart(canvas, program) {
    ...
    ctx.beginPath();
    ctx.moveTo(34, 10);
    ctx.lineTo(34, el.height - 28);
    ctx.lineTo(el.width - 10, el.height - 28);
    ctx.stroke();
    ctx.fillStyle = '#64748b';
    ctx.font = '12px Malgun Gothic, sans-serif';
    ctx.fillText('최근 점수', 10, 16);
    if (!rows.length) {
      ctx.fillText('아직 기록이 없습니다.', 48, 64);
      return;
    }
    const barW = Math.max(12, (el.width - 60) / rows.length - 6);
    rows.forEach((row, i) => {
      const x = 42 + i * (barW + 6);
      const h = Math.max(2, (el.height - 48) * (row.score / 100));
      const y = el.height - 28 - h;
      ctx.fillStyle = '#10b981';
      ctx.fillRect(x, y, barW, h);
      ctx.fillStyle = '#334155';
      ctx.fillText(String(row.score), x, y - 4);
    });
  }

  function summary(program) {
    const data = load();
    const due = reviewWords(program).length;
    const sessions = history(program, 9999);
    const last = sessions[sessions.length - 1];
    return { due, totalSessions: sessions.length, lastScore: last ? last.score : null };
  }
